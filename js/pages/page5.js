...
(function() {
    ...
    function renderBarLineChart() {
        ...
        const option = {
            ...
            xAxis: [{ 
                type: 'category', 
                data: batches, 
                axisLabel: { interval: 0, rotate: 30, ...FONT_OPTION } 
            }],
            yAxis: [
                { 
                    type: 'value', 
                    name: '降幅 (%)', 
                    axisLabel: { ...FONT_OPTION }, 
                    nameTextStyle: FONT_OPTION 
                },
                { 
                    type: 'value', 
                    name: '品种数', 
            ...
            series: [
                { 
                    name: '平均降幅', 
                    type: 'bar', 
                    itemStyle: { color: COLOR_PALETTE.primary }, 
                    data: procurementData.map(d => d.avg_drop) 
                },
                { 
                    name: '最高降幅', 
                    type: 'bar', 
                    itemStyle: { color: COLOR_PALETTE.secondary }, 
                    data: procurementData.map(d => d.max_drop) 
                },
                { 
                    name: '招标品种数', 
                    type: 'line', 
                    yAxisIndex: 1, 
                    smooth: true, 
                    itemStyle: { color: COLOR_PALETTE.accent }, 
                    lineStyle: { width: 3 }, 
                    data: procurementData.map(d => d.bid) 
                }
            ],
        ...
        const option = {
            ...
            series: [
                { 
                    ...
                    type: 'line', 
                    stack: 'Total', 
                    areaStyle: { color: COLOR_PALETTE.base }, 
                    showSymbol: false, 
                    smooth: 0.5, 
                    data: procurementData.map(d => d.win) 
                },
                { 
                    name: '平均降幅', 
                    type: 'line', 
                    ...
                    smooth: true, 
                    showSymbol: false, 
                    lineStyle: { width: 3, color: COLOR_PALETTE.accent }, 
                    data: procurementData.map(d => d.avg_drop) 
                }
            ],
            graphic: { 
                elements: [{ 
                    type: 'image', 
                    id: 'rollercoaster-icon', 
                    style: { 
                        image: 'https://youke1.picui.cn/s1/2025/08/17/68a0b37b5a6d5.png', 
                        width: 40, 
                        height: 40 
                    }, 
                    invisible: true 
    ...
    function renderTimelineChart(alignStart = false) {
        ...
        const data = procurementData.map((item, index) => {
             ...
             return { 
                 name: item.batch, 
                 value: alignStart ? [index, 0, duration, item.avg_drop] : [index, startDate, execDate, item.avg_drop] 
             };
        });
        ...
        const option = {
            title: { 
                text: '药品集采时间跨度与降价力度', 
                subtext: alignStart ? '按天数比较周期时长 (从招标到执行)' : '按真实时间查看执行窗口 (从招标到执行)', 
                left: 'center' 
            },
            tooltip: { 
                formatter: p => `${procurementData[p.value[0]].batch}<br/>时长: ${(((new Date(procurementData[p.value[0]].exec_date)) - (new Date(procurementData[p.value[0]].start_date))) / (1000 * 60 * 60 * 24)).toFixed(0)} 天<br/>平均降幅: <b>${p.value[3]}%</b>` 
            },
            grid: { 
                top: '22%', 
                left: '5%', 
    ...
    function toggleGanttView() { 
        ...
        document.getElementById('gantt-toggle-btn').innerText = isGanttAligned ? '返回时间视图' : '对齐起点比较'; 
    ...
    function switchTab(index) {
        ...
        document.querySelectorAll('.chart-wrapper').forEach((w, i) => w.classList.toggle('active', i === index));
    ...
    function handleLogoClick() {
        const activeChartWrapper = document.querySelector('.chart-wrapper.active');
    ...
    function init() {
        ...
        // 绑定事件
        document.getElementById('logo-container').addEventListener('click', handleLogoClick);
    ...
    window.Page5 = {
        ...
        switchTab: switchTab,
        toggleGanttView: toggleGanttView
    };
